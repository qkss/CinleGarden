const cx=new Proxy({},{get:(t,p)=>p in t?t[p]:(()=>{}),set:(t,p,v)=>{t[p]=v;return true;}});
const ch={};function mk(){return{width:980,height:620,getContext:()=>cx,addEventListener:(e,f)=>{ch[e]=f;},getBoundingClientRect:()=>({left:0,top:0,width:980,height:620})};}
let ov='';let sf=null;
...
const fs=require('fs');(0,eval)(fs.readFileSync('gamedbg.js','utf8'));sf();
const click=(x,y)=>ch['click']({clientX:x,clientY:y});const CARD={sunflower:200,peashooter:332,wallnut:464};const cell=(c,r)=>({x:60+c*96+48,y:110+r*92+46});
...
let log=[];
for(let f=0;f<170*fps;f++){t+=dt;
 for(let i=sch.length-1;i>=0;i--){if(sch[i].at<=t){sch[i].fn();sch.splice(i,1);}}
 if(raf){const c=raf;raf=null;c(t);} collect();
 const sec=t/1000;
 if(f%20===0){for(let r=0;r<5;r++)plant('sunflower',0,r);
 ...
 if(f%(fps*5)===0&&global.__G)log.push(`t${global.__G.t.toFixed(0)} sun${global.__G.sun} pl${global.__G.plants} z${global.__G.zombies} spawned${global.__G.zSpawned}`);
 if(ov.includes('胜利')||ov.includes('游戏结束'))break;
}
console.log(log.join('\n'));console.log('END',global.__G&&global.__G.state, ov.includes('胜利')?'WON':ov.includes('游戏结束')?'LOST':'-');
