...
const click=(x,y)=>ch['click']({clientX:x,clientY:y});const keydown=k=>ch['win_keydown']({key:k,code:'',preventDefault(){}});
...
for(let f=0;f<300*60;f++){t+=dt;
 ...
 if(f%30===0){ ch['win_keydown']({shiftKey:true,code:'Digit0',key:')',preventDefault(){}});  // +sun
   for(let r=0;r<5;r++){plant('sunflower',0,r);plant('potatoshield',1,r);plant('threepeater',2,r);plant('snowpea',3,r);}
   for(const p of A.plants){ if(p.type==='potatoshield'||p.type==='sunflower'){ const cc=cell(p.c,p.r); click(cc.x,cc.y); } }
 }
 if(ov.includes('游戏结束')) break;
}
const psMax=Math.max(0,...A.plants.filter(p=>p.type==='potatoshield').map(p=>p.up));
console.log('errors:', err.length?err.slice(0,4):'none', '| potatoshield maxUp seen:', psMax);
