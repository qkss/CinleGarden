...
// place potato shields at levels 0,2,4,6,8,10 across columns; rows cycle
const lv=[0,2,4,6,8,10];
for(let i=0;i<lv.length;i++){ const r=i%5, c=i; A.addPlant('potatoshield',r,c); const p=A.plants[A.plants.length-1]; p.up=lv[i]; p.selfHpMult=1+0.5*lv[i]; }
...
if(raf)raf(t);
fs.writeFileSync('preview19.png', real.toBuffer('image/png'));
console.log('done levels:', lv);
