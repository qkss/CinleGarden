...
(0,eval)(fs.readFileSync('gamecheat5.js','utf8'));
const A=global.__API; A.start();
A.addPlant('threepeater',1,1); A.addPlant('peashooter',2,1); A.addPlant('repeater',3,1);
A.addZombie('basic',1); A.addZombie('basic',2); A.addZombie('basic',3);
const dt=1000/60; for(let f=0;f<70;f++){t+=dt;for(let i=sch.length-1;i>=0;i--){if(sch[i].at<=t){sch[i].fn();sch.splice(i,1);}}if(raf){const c=raf;raf=null;c(t);}}
if(raf)raf(t);
fs.writeFileSync('preview15.png', real.toBuffer('image/png'));
console.log('done');
