...
let store={};
global.document={getElementById:id=>({game:gameEl,overlay:{classList:{add(){},remove(){}},set innerHTML(v){store.ov=v;},get innerHTML(){return store.ov||"";}},startBtn:{set onclick(f){}},againBtn:{set onclick(f){}}}[id]||{set onclick(f){}, addEventListener(){}})};
...
const A=global.__API; A.start();   // hides overlay, runs game
A.addPlant('sunflower',0,0); A.addPlant('peashooter',2,2);
...
if(raf)raf(t);
fs.writeFileSync('preview28.png', real.toBuffer('image/png'));
console.log('done');
