...
const base={createRadialGradient:()=>grad,createLinearGradient:()=>grad};
...
(0,eval)(require('fs').readFileSync('game.js','utf8'));sf();
const click=(x,y)=>ch['click']({clientX:x,clientY:y});
const cell=(c,r)=>({x:60+c*96+48,y:130+r*92+46});
const CARDX=i=>130+i*90+43;
const key={sunflower:0,peashooter:1,snowpea:2,repeater:3,wallnut:4,cherrybomb:5,potatomine:6};
...
for(let f=0;f<240*fps;f++){t+=dt;
 ...
 if(f%20===0){
   ...
   if(sec>8)for(let r=0;r<5;r++)plant('peashooter',3,r);
   if(sec>30)for(let r=0;r<5;r++){plant('peashooter',2,r);plant('snowpea',4,r);}
   if(sec>60)for(let r=0;r<5;r++){plant('repeater',5,r);plant('wallnut',6,r);}
...
console.log('errors:',err.length?err.slice(0,4):'none','| won:',won,'lost:',lost,'| t:',(t/1000).toFixed(1)+'s');
